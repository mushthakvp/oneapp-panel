...
import { useSearchSubcategory } from '../../../../../api/useDataControllerAdmin';
...
function SubSection({ selected, setSelected }) {
    ...
    const { isLoading, data, error } = useSearchSubcategory(
      search,
      selected?.category
    );
  ...
  return (
    <div
      onClick={(e) => {
        e.stopPropagation();
        if(!selected?.category)return toast.error('Please select category')
        setIsOpen(!isOpen);
      }}
