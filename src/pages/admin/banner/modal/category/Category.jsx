...
import { useSearchCategory } from '../../../../../api/useDataControllerAdmin';
import { toast } from 'react-toastify';

function Category({selected, setSelected,sectionId}) {
    const [isOpen, setIsOpen] = React.useState(false);
    const [search,setSearch]=useState('')
    const { data, isLoading, error } = useSearchCategory(search,sectionId);
  ...
  return (
    <div
      onClick={() => {
        if (!sectionId) {
          toast.error("Please Select section first");
          return
       }
        setIsOpen(!isOpen)
      }}
      className="flex justify-between items-center w-full h-10 gap-[8px] transition-all duration-700 ease-out"
    >
      <div className="w-full relative border border-inputBorder rounded-md h-10 flex items-center justify-between px-3">
            ...
            {data?.categories?.length > 0 &&
              data?.categories?.map((data) => {
                return (
                  <li
                    key={data._id}
                    className="flex justify-between items-center w-full h-10 px-2 hover:bg-inputBorder rounded-md cursor-pointer"
                    onClick={() => {
