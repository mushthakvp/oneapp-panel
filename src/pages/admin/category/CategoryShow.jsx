...
function CategoryShow() {
  ...
  const { data, isLoading, error } = useGetCategory(page);
  ...
  const handleDelete = () => {
    mutate({ status: 'delete', id: deleteId },
      {
        onSuccess: (data) => {
          setDeleteId('')
  ...
  return (
    <div>
      ...
      <div className="bg-containerWhite mt-[15px] p-[22px] rounded-md min-w-[750px] overflow-x-auto">
        ...
        {isLoading?<TableShimmer rowCount={5} columnCount={3}/>:  <tbody>
      ...
      {data?.categories.length===0 &&!isLoading&& <NodataWithText text="No Category Found" subText={'Category'}/>}
      {data?.totalPages > 1 && (
