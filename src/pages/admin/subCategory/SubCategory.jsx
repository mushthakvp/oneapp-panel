...
import { useAddAdminSubCategory, useGetSubCategory } from '../../../api/useDataControllerAdmin';
...
const SubCategory = () => {
    ...
    const { data, isLoading, error } = useGetSubCategory(page);
    const [hoverIndex, setHoverIndex] = useState('')
    const [hoverDeleteIndex, setHoverDeleteIndex] = useState("");
    const { mutate, isPending } = useAddAdminSubCategory();
    const [deleteId, setDeleteId] = useState('')

    const handleDelete = () => {
        mutate({ action: 'delete', id: deleteId },
            {
                onSuccess: (data) => {
                    setDeleteId('')
    ...
    return (
        <div>
            ...
            <div className="bg-containerWhite mt-[15px] p-[22px] rounded-md min-w-[750px] overflow-x-auto">
                ...
                {/* table  */}
                <table className="text-left w-full mt-4 min-w-[700px]">
                    <thead className="bg-[#F8FAFC] rounded-r-lg">
                        <th className="text-[14px] px-3 p-2 font-[14px] opacity-60 rounded-l-lg">
                            Subcategories
                        </th>
                    ...
                    {isLoading ? <TableShimmer rowCount={8} columnCount={4} /> : <tbody>
                        {data?.subCategories?.length > 0 &&
                            data?.subCategories?.map((data, index) => {
                                return (
                                    <tr>
                                        <td className="text-[14px] px-3 p-2 font-[14px]">
                                        ...
                                        <td className="text-[14px] px-3 p-2 font-[14px]">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <button
                                                    ...
                                                    onMouseLeave={() => setHoverDeleteIndex("")}
                                                >
                                                    {hoverDeleteIndex !== index ? (
                                                        <svg
                                                            width="40"
                                                            height="40"
                                                            viewBox="0 0 40 40"
            ...
            {data?.subCategories?.length === 0 && !isLoading && <NodataWithText text="No Subcategory Found" subText={'Subcategory'} />}
            {data?.totalPages > 1 && (
