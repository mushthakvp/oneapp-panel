...
function UnblockModal({ isOpen, setIsOpen, vendor }) {
    ...
    takeAction(
      { payload, action: 'unblock' },
      {
        ...
        onError: (error) => {
          ...
          toast.error(error?.message || 'Unblocking failed. Please try again.');
        },
      }
    );
  };
      ...
      <div className="outline-none text-center w-full max-w-[332px] rounded-md bg-containerWhite flex flex-col justify-center items-center gap-5 p-4">
        ...
        <div>
          ...
          <p className="text-[12px] font-[400] mt-[7px]">
            Are you sure want to unblock the Vendor
          </p>
        </div>
        <div className="flex items-center gap-2 w-full">
          ...
          <button onClick={handleUnblock} className="text-sm w-full h-[35px] flex items-center justify-center rounded-md text-white bg-buttonColor border border-buttonColor">
            Yes
          </button>
        </div>
      </div>
